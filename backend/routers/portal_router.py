...
async def cancellations(pu: dict = Depends(get_portal_user)):
    ...
    Fase 56C. Yang wajib jujur di sini: berapa yang sudah diterima pengembang, berapa yang
    dipotong beserta DASAR ATURANNYA, berapa yang dikembalikan, apa yang sedang ditunggu
    (mis. unit terjual kembali), dan berapa yang sudah dibayarkan. Tanpa itu pembeli hanya
    tahu "kontrak saya batal" tanpa bisa menagih haknya.
    """
    org = pu.get("org_id", ORG_ID)
    cust = await _customer(pu)
    deals = await _deals(pu, cust)
