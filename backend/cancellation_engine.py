...
async def _void_ar(org: str, cancel: dict) -> dict:
    ...
    await db.ar_invoices.update_one({"id": inv["id"]}, {"$set": {
        "items": items, "status": "cancelled", "outstanding": 0,
        "cancelled_at": ts, "cancel_reason": cancel.get("reason"),
        "cancellation_id": cancel["id"], "updated_at": ts}})
...
async def refund_hold(org: str, cancel: dict) -> dict:
    ...
    if (cancel.get("settlement") or {}).get("refund_requires_resale"):
        unit = await db.units.find_one({"id": cancel.get("unit_id")}, {"_id": 0}) or {}
        terjual_ulang = bool(unit.get("reserved_by_deal") or unit.get("booked_by_deal")
                             or unit.get("deal_id"))
